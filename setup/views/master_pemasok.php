...
<div class="content-header">   
	<h4><?php echo $setting->first_row()->label_pemasok?></h4>
</div>
				...
				<tbody>
					<?php $seq = 1;
						...
						<tr>
							...
							<td><?php echo $row->alamat_pemasok?></td>
							<td><?php 
							...
							<td>
								<input type="hidden" class="form-control" id="id_pemasok" name="id_pemasok" role="inputtext" value="<?php echo $row->id_pemasok; ?>" />
								<button onclick="editdata(this,<?php echo $row->id_pemasok ?>)" class="btn btn-xs btn-warning">
									<span class="glyphicon glyphicon-pencil"></span>
								</button>
								<button onclick="deletedata(this,<?php echo $row->id_pemasok ?>)" class="btn btn-xs btn-danger">
									<span class="glyphicon glyphicon-trash"></span>
								</button>
							</td>
						</tr>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header" style="padding:10px 10px 0px 0px !important;">
        ...
        <h5 style="padding: 0px 15px 0px 15px;">Tambah <?php echo $setting->first_row()->label_pemasok?></h5>
      </div>
	        ...
	        <div class="col-md-12">
				<div style="background:#429489;min-height:0px;" class="widget-container padded">
					...
					<div class="buttonfooter">
						<button class="btn btn-xs btn-success"  onclick="simpanpemasok()" type="button">
...
<div class="modal fade" id="modalubah">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header" style="padding:10px 10px 0px 0px !important;">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>    
        <h5 style="padding: 0px 15px 0px 15px;">Ubah Data <?php echo $setting->first_row()->label_pemasok?></h5>
      </div>
      <div class="modal-body">
        <div class="row">
				...
				<div style="background:#429489;min-height:0px;" class="widget-container padded">
					<table class="table table-bordered tsble-striped" style="background:#fff;">
						<tr>
							...
							<td class="form-input">
								<div class="col-md-12">
									<input type="hidden" class="form-control" name="edit_id_pemasok" id="edit_id_pemasok" role="inputtext" />
					...
					<div class="buttonfooter">
						<button class="btn btn-xs btn-success"  onclick="simpanpemasokubah()" type="button">
							<span class="glyphicon glyphicon-save"></span> Simpan
						</button>
...
<script type="text/javascript">
	...
	function simpanpemasok(){
		//alert('ok');
  	 	var target="<?php echo site_url('setup/simpanpemasok')?>";
  	 	...
  	 	$.post(target,data,function(e){
  	 		...
  	 		if(jojon.flag)
  	 		{
  	 			$('#datatable').dataTable().fnAddData( [
                    ...
                    jojon.email,
                    '<input type="hidden" class="form-control" id="id_pemasok" name="id_pemasok" role="inputtext" value="'+jojon.newID+'" /><button onclick="editdata(this,'+jojon.newID+')" class="btn btn-xs btn-warning"><span class="glyphicon glyphicon-pencil"></span></button> <button onclick="deletedata(this,'+jojon.newID+')" class="btn btn-xs btn-danger"><span class="glyphicon glyphicon-trash"></span></button>' ] );
	...
	function deletedata(obj, IDP)
	{
		...
		if(isdelete)
		{
			...
			var target="<?php echo site_url('setup/hapuspemasok')?>";
				data={id_pemasok:IDP};
				row     = $('#datatable').find('#id_pemasok[value="'+IDP+'"]').parent().parent();
			$.post(target,data,function(e){
				var jojon 	= $.parseJSON(e);
	...
	function simpanpemasokubah()
	{
		var target = "<?php echo site_url("setup/simpanpemasokubah")?>";
			...
			$.post(target, data, function(e)
			{
				...
				var row 	= $('#datatable').find('#id_pemasok[value="'+idNye+'"]').parent().parent();
