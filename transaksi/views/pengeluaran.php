			...
			<div class="row" style="border-bottom:1px dashed #ccc; padding-bottom:10px; margin-bottom:10px;">
				<div class="btn">
					<a href="javascript:toggleDiv('myContent');" class="btn btn-sm btn-success">
						<span class="glyphicon glyphicon-list"></span>&nbsp;Pencarian BKK
					</a>
				</div>
				<div id="myContent">
				...
				<tbody id="tabpengeluaran">
					...
					<tr>
						...
						<td>
							...
							<a href="transaksi/printkuitansiBKK/<?php echo $row->id_pembelian ?>"  target="_blank" style='margin-left:3px;' class='btn btn-xs btn-primary' ><span class='glyphicon glyphicon-print' aria-hidden='true'></a>
						</td>
					</tr>
					<?php 
					$seq++;
...
<script type="text/javascript">
	...
	function tambahdata()
	{
		...
		var target = "<?php echo site_url("transaksi/tambahpengeluaran")?>";
	...
	function editdata(obj)
	{
		...
		var target = "<?php echo site_url("transaksi/getdataeditpengeluaran")?>";
		data = {
	...
	function deletedata(obj)
	{
		...
		var target = "<?php echo site_url("transaksi/deletepengeluaran")?>";
		data = {
			idpengeluaran : IDx
		}
		// console.log(IDx);
	...
	function deleteall()
	{
		...
		if (ids.length) {
			...
			if(conf)
			{
				$.ajax({
					type: 'POST',
					url: "<?php echo site_url("transaksi/deletepengeluaranall")?>",
					data: {
						idpenjualan: ids
					},
...
<<<<<<< .mine
			ColButt.innerHTML 		= '<button onclick="editdata(this)" class="btn btn-xs btn-warning"><span class="glyphicon glyphicon-pencil"></span></button> <button onclick="deletedata(this)" class="btn btn-xs btn-danger"><span class="glyphicon glyphicon-trash"></span></button><a href="transaksi/printkuitansiBKK/'+idpembelian+'"  target="_blank" style="margin-left:3px;"" class="btn btn-xs btn-primary" ><span class="glyphicon glyphicon-print" aria-hidden="true"></a>';
||||||| .r149
