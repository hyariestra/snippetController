					...
					<div class="form-horizontal">
						...
						<div class="col-md-6">
							...
							<div class="form-group">
								...
								<div class="col-md-8">
								<input type="hidden" class="form-control" name="namaunit"/>
								<input type="hidden" class="form-control" name="namabank"/>
								<input type="hidden" class="form-control" name="bulan"/>
								<input type="hidden" class="form-control" name="tahun"/>
								<div class="input-group">
									<span class="input-group-addon" id="namaunit_text"></span>
									<span class="input-group-addon" id="namabank_text"></span>
									<span class="input-group-addon" id="bulan_text"></span>
									<span class="input-group-addon" id="tahun_text"></span>
						...
						<div class="col-md-6">
							...
							<div class="form-group">
								...
								<div class="col-md-8">
									<div class="input-group">
										<input type="text" id="nama_pemasok"  readonly class="form-control"/>
										<input type="hidden" readonly class="form-control" id="id_pemasok" name="id_pemasok"/>
					...
					<table class="table table-bordered">
						...
						<tbody id="dataitem">
							<tr>
								...
								<td><input type="number" value="1" onblur="akumulasi(this)" role="inputtext" class="form-control" name="jmlitem[]" /></td>
								<td>
									<div class="input-group">
										<span class="input-group-addon">Rp. </span>
										<input type="text"  role="inputtext" style="text-align:right;" onblur="akumulasi2(this)" onkeyup='formatNumber(this)' class="form-control" name="harga[]" />
								...
								<td>
									<div class="input-group">
										<span class="input-group-addon">Rp. </span>
										<input type="text" value="0,00" style="text-align:right;" readonly onblur="akumulasi2(this)" onkeyup="formatNumber(this)" role="inputtext" class="form-control" name="potongan[]" />
									</div>
								</td>
								<td><input type="text" role="inputtext" class="form-control" name="memo[]" /></td>
								<td>
									<div class="input-group">
										<span class="input-group-addon">Rp. </span>
										<input type="text" value="0,00"  style="text-align:right;" readonly onkeyup="formatNumber(this)" role="inputtext" class="form-control" name="subtotal[]" />
									</div>
								</td>
								<td>
...
<div class="modal fade" id="modalTambahPemasok">
							...
							<form id="formtambahpemasok">
								<table class="table table-bordered table-striped" style="background:#fff;">
									...
									$idakun = $this->db->query("SELECT mst_akun.id_akun 
										FROM mst_akun WHERE mst_akun.kode_akun = '5' 
	...
	$(document).ready(function(){
		...
		.on('changeDate', function(e){
			...
			getautonumberchange(kasbank, bulan, tahun);
			
		});

		$('#datatable').DataTable();
		getnumbertransaksipengeluaran();
	...
	function getautonumberchange(kasbank, bulan, tahun)
	{
		var target = "<?php echo site_url("transaksi/getautonumberchangePengeluaran")?>";
			data = {
				kasbank : kasbank,
				bulan : bulan,
				tahun : tahun
		...
		$.post(target, data, function(e){
			...
			$("input[name='namaunit']").val(json.notrans.unit);
			$("input[name='namabank']").val(json.notrans.bank);
			$("input[name='bulan']").val(json.notrans.bulan);
			$("input[name='tahun']").val(json.notrans.tahun);
			$("input[name='no_transaksi']").val(json.notrans.nourut);
			
			
			$("#namaunit_text").text(json.notrans.unit);
			$("#namabank_text").text(json.notrans.bank);
			$("#bulan_text").text(json.notrans.bulan);
			$("#tahun_text").text(json.notrans.tahun);
			$("input[name='no_transaksi']").text(json.notrans.nourut);
		});
	}
	...
	function tambahrow()
	{
		...
		Harga += "<input class='form-control' style='text-align:right;' onblur='akumulasi2(this)' onkeyup='formatNumber(this)' role='inputtext' name='harga[]' />";
		Harga += "</div>";

		var Potongan = "<div class='input-group'>";
		Potongan += "<span class='input-group-addon'>Rp. </span>";
		Potongan += "<input class='form-control' style='text-align:right;' readonly onblur='akumulasi2(this)' role='inputtext' value='0,00' onkeyup='formatNumber(this)' name='potongan[]' />";
		Potongan += "</div>";
		...
		Total += "<input class='form-control' style='text-align:right;' readonly role='inputtext' value='0,00' onkeyup='formatNumber(this)' name='potongan[]' />";
		Total += "</div>";
	...
	function openpemasok()
	{
		...
		$.post(target, data, function(e){
			...
			{
				alert("Silahkan isi data pemasok terlebih dahulu.");
			}
		});
	}
...
function getnumbertransaksipengeluaran()
{
...
function simpandata()
{
	...
	var pelanggan = $("input[name='id_pelanggan']").val();
	var table = $("tbody#dataitem").find("tr");

	var flag = false;

	if(metodebayar == "-" || bank == "-" || ket == "" || pelanggan == "")
	{
	...
	$.each(table, function(i, v){
		...
		//var x = $("#idbank option:selected").html();
	...
	var target = "<?php echo site_url("transaksi/simpandatapengeluaran")?>"
	data = {
		...
		$.post(target, data, function(e){
			...
			var tableRow = $("tbody#dataitem").find("tr");
...
/*	var number = num[0]+'/'+x+'/'+num[2]+'/'+num[3]+'/'+num[4];*/
}
...
function caridata() {
	...
	var target = "<?php echo site_url("transaksi/cari_mustahiq2")?>";
	data = {
		mustahiq : nilai
	}
...
function cetaksimpan()
{
	...
	var target = "<?php echo site_url("transaksi/simpandancetak")?>";
	...
	$.ajax({
		...
		success(result,status,xhr){
			...
			var loadContent = "<?php echo site_url("transaksi/pengeluaran")?>";
...
function daftar()
{
	...
	var target = "<?php echo site_url("transaksi/pengeluaran")?>";
