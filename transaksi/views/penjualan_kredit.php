...
<h4>Transaksi Penerimaan Secara Kredit</h4>
			...
			<div class="row" style="border-bottom:1px dashed #ccc; padding-bottom:10px; margin-bottom:10px;">
				<div class="btn">
					<a href="javascript:toggleDiv('myContent');" class="btn btn-sm btn-success">
						<span class="glyphicon glyphicon-list"></span>&nbsp;Daftar BKM Penerimaan Kredit
					</a>
				</div>
			...
			<table id="datatable" class="table table-bordered table-striped" style="margin-top:10px;">
				...
				<tbody id="tabpenjualan">
					...
					<tr>
						...
						<td>
							<button onclick="editdatakredit(this)" class="btn btn-xs btn-warning">
								<span class="glyphicon glyphicon-pencil"></span>
							</button>
							<button onclick="deletedata(this)" class="btn btn-xs btn-danger">
								<span class="glyphicon glyphicon-trash"></span>
							</button>
							<button onclick="printdatakuitansi('<?php echo $row->id_penjualan?>')" class="btn btn-xs btn-primary">
...
<script type="text/javascript">
	...
	function printdata()
	{
		var target = "<?php echo site_url("transaksi/printrekappenjualankredit")?>";
	...
	function tambahdata()
	{
		...
		var target = "<?php echo site_url("transaksi/tambahpenjualankredit")?>";
		var aa = 'aa';
	...
	function editdatakredit(obj)
	{
		$("body").append("<div style='position:fixed !important;' class='backDropOverlay' id='backDropOverlay'><div><img src='assets/images/loading.gif'/><span>Loading..</span></div></div>");
		/*var aa = 'aa';*/
		var target = "<?php echo site_url("transaksi/editdatabkmkredit")?>";
	...
	function deletedata(obj)
	{
		...
		//var va 	= 	 $(obj).parent().parent().find('td:eq(6) input').val();
		...
		if(isdelete)
		{
			$.post(target+"?na="+na, data, function(e){
	...
	function deleteall()
	{
		...
		if (ids.length) {
			$.ajax({
				type: 'POST',
				url: "<?php echo site_url("transaksi/deleteall?na='+na'")?>",
				data: {
					idpenjualan: ids
				},
			...
			//window.open(target+"?na="+na,"_blank")
	...
	function inggridpangalila(data)
	{
		...
		for(var x=0; x < data.length; x++)
		{
			...
			var ColKet 		= row.insertCell(5);
			var ColNom 		= row.insertCell(6);
			...
			ColButt.innerHTML 		= '<button onclick="editdatakredit(this)" class="btn btn-xs btn-warning"><span class="glyphicon glyphicon-pencil"></span></button> <button onclick="deletedatapenjualan(this)" class="btn btn-xs btn-danger"><span class="glyphicon glyphicon-trash"></span></button><a href="transaksi/printkuitansi/'+idpenjualan+'"target="_blank" style="margin-left:3px;" class="btn btn-xs btn-primary" ><span class="glyphicon glyphicon-print" aria-hidden="true"></a>';
	...
	function printdatakuitansi(idx)
	{
		var target = "<?php echo site_url("transaksi/printkuitansikredit")?>/"+idx;
