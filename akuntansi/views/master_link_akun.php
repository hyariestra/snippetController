		...
		<div class="row" style="border-bottom:1px dashed #ccc; padding-bottom:10px; margin-bottom:10px;">
			<div class="col-md-6">
				<form id="formlinkakun" class="form-horizontal">
					...
					<?php 
					$arr = 0;
					foreach($linkakun->result() as $row){ ?>
					<input type="hidden" name="id_ref_akun[]" value="<?php echo $row->id_akunlink ?>">
						<div class="form-group">
							<label class="control-label col-md-4"><?php echo $row->nama_akunlink?></label>
							<div class="col-md-8">
								<div class="input-group">
									<span class="input-group-addon"><?php echo @$row->kode_induk.".".@$row->kode_akun ?></span>
									<input type="hidden" id="akunref_<?php echo $arr?>" value="<?php echo @$row->kode_induk ?>" class="form-control" name="id_akun[]" />
									<input type="text" class="form-control" value="<?php echo @$row->nama_akun?>" />
				...
				<div style="background:#429489;min-height:0px;" class="widget-container padded">
					...
					<table class="table table-bordered table-striped" style="background:#fff;">
						...
						<?php
							...
							foreach($akun->result_array() as $akunrows)
							{
						?>
						<tr>
							<td>
								<?php echo $akunrows['kode_induk'].".".$akunrows['kode_akun'] ?>
								<input type="hidden" value="<?php echo $akunrows['id_akun'] ?>" />
								<input type="hidden" value="<?php echo $akunrows['kode_induk'].".".$akunrows['kode_akun'] ?>" />
							</td>
							<td><?php echo $akunrows['nama_akun']?></td>
							<td>
								<?php
								if($akunrows['level'] == 4){
								?>
...
<script type="text/javascript">
	...
	function simpandata()
	{
		...
		var target = "<?php echo site_url("akuntansi/simpandatalinkakun")?>";
