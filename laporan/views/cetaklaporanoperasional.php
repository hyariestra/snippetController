...
<div class="row">
	<div class="col-md-12">
		<div style="text-align:center; margin-bottom: 15px !important;">
			<h4>Laporan <?php echo $label->first_row()->label_header?></h4>
			...
			<div class="content-laporan">
				<table class="table table-bordered table-stripped">
					...
					<?php
						...
						{
							if($akun->level <= 3 && $akun->kodePlainText != "4.1.99")
							{
								$space = getSpace($akun->level);
								$nominal = ($akun->level == 3) ? formatCurrency(GetTotalAkun($akun->kodePlainText, $akun->saldoNormal, $akun->level, $startDate, $endDate, $unit)) : "";
					?>
								<tr>
					...
					<?php
								$total += GetTotalAkun($akun->kodePlainText, $akun->saldoNormal, $akun->level , $startDate, $endDate, $unit);
							}
						}
					?>
					<tr>

						<td><b>TOTAL PENDAPATAN <?php echo $label->first_row()->label_laporan?></b></td>
					...
					<?php
						...
						{
							if($akunBiaya->level <= 3  && $akunBiaya->kodePlainText != "5.1.99")
							{
								$space = getSpace($akunBiaya->level);
								$nominal = ($akunBiaya->level == 3) ? formatCurrency(GetTotalAkun($akunBiaya->kodePlainText, $akunBiaya->saldoNormal, $akunBiaya->level, $startDate, $endDate, $unit)) : "";
					?>
								<tr>
					...
					<?php
							$totalbiaya += GetTotalAkun($akunBiaya->kodePlainText, $akunBiaya->saldoNormal, $akunBiaya->level, $startDate, $endDate, $unit);
							}
						}
					?>
					<tr>
						<td><b>TOTAL BEBAN <?php echo $label->first_row()->label_laporan?></b></td>
					...
					?>
					<tr>
						<td><b><?php echo $label->first_row()->label_tambahan?> DARI KEGIATAN <?php echo $label->first_row()->label_laporan?></b></td>
					...
					<!-- OPERASIONAL -->
					<tr>
						<td><b>PENDAPATAN NON <?php echo $label->first_row()->label_laporan?></b></td>
						<td></td>
					</tr>
					<tr>
						<td style="padding-left:20px"><b>PENDAPATAN NON <?php echo $label->first_row()->label_laporan?></b></td>
						<td></td>
					</tr>
					<?php
					...
					?>
								<tr>
									<td style="<?php echo $space?>"><?php echo $akunnonops->namaPlainText?></td>
									<td style="text-align:right;"><?php echo formatCurrency(GetTotalAkunNonOps($akunnonops->kodePlainText, $akunnonops->saldoNormal, $akunnonops->level, $startDate, $endDate, $unit)) ?></td>
								</tr>
					<?php
								$totalnonops += GetTotalAkunNonOps($akunnonops->kodePlainText, $akunnonops->saldoNormal, $akunnonops->level, $startDate, $endDate, $unit);
							}
						}
					?>
					
					<tr>
						<td><b>TOTAL PENDAPATAN NON <?php echo $label->first_row()->label_laporan?></b></td>
						<td style="text-align:right; font-weight:bold;"><?php echo formatCurrency($totalnonops) ?></td>
					</tr>
					
					<!-- BIAYA NON OPERASIONAL -->
					<tr>
						<td><b>BEBAN NON <?php echo $label->first_row()->label_laporan?></b></td>
						<td></td>
					</tr>
					<tr>
						<td style="padding-left:20px"><b>BEBAN NON <?php echo $label->first_row()->label_laporan?></b></td>
						<td></td>
					</tr>
					<?php
					...
					?>
								<tr>
									<td style="<?php echo $space?>"><?php echo $akunbiayanonops->namaPlainText?></td>
									<td style="text-align:right;"><?php echo formatCurrency(GetTotalAkunNonOps($akunbiayanonops->kodePlainText, $akunbiayanonops->saldoNormal, $akunbiayanonops->level, $startDate, $endDate, $unit)) ?></td>
								</tr>
					<?php
								$totalbiayanonops += GetTotalAkunNonOps($akunbiayanonops->kodePlainText, $akunbiayanonops->saldoNormal, $akunbiayanonops->level, $startDate, $endDate, $unit);
							}
						}
					?>
					<tr>
						<td><b>TOTAL BEBAN NON <?php echo $label->first_row()->label_laporan?></b></td>
					...
					<tr>
						...
						<td><b><?php echo $label->first_row()->label_tambahan?> DARI KEGIATAN NON <?php echo $label->first_row()->label_laporan?></b></td>
					...
					?>
					<tr>
						<td><b><?php echo $label->first_row()->label_tambahan?></b></td>
...
<?php
	...
	function GetTotalAkun($kodeakun, $saldonormal, $level, $startDate, $endDate, $unit)
	{
		$CI=&get_instance(); 
		
		$filterunit = ($unit != 1) ? " AND mst_akun.level > 3 AND mst_akun.id_unit = '".$unit."'" : "";
		...
		WHERE trx_jurnal_det.id_akun IN (SELECT mst_akun.id_akun FROM mst_akun WHERE CONCAT(mst_akun.kode_induk,'.',mst_akun.kode_akun) LIKE '".$kodeakun."%' $filterunit)
		AND (trx_jurnal.tgl_jurnal >= '".$startDate."' AND trx_jurnal.tgl_jurnal <= '".$endDate."') 
		AND 
		(trx_jurnal_det.id_akun NOT IN (SELECT mst_akun.id_akun FROM mst_akun WHERE CONCAT(mst_akun.kode_induk,'.',mst_akun.kode_akun) LIKE '5.1.99%')
		AND  
		trx_jurnal_det.id_akun NOT IN (SELECT mst_akun.id_akun FROM mst_akun WHERE CONCAT(mst_akun.kode_induk,'.',mst_akun.kode_akun) LIKE '4.1.99%'))
		");
	...
	function GetTotalAkunNonOps($kodeakun, $saldonormal, $level, $startDate, $endDate, $unit)
	{
		$CI=&get_instance(); 
		...
		AND (trx_jurnal.tgl_jurnal >= '".$startDate."' AND trx_jurnal.tgl_jurnal <= '".$endDate."')
		AND 
		(trx_jurnal_det.id_akun IN (SELECT mst_akun.id_akun FROM mst_akun WHERE CONCAT(mst_akun.kode_induk,'.',mst_akun.kode_akun) LIKE '5.1.99%')
		OR  
		trx_jurnal_det.id_akun IN (SELECT mst_akun.id_akun FROM mst_akun WHERE CONCAT(mst_akun.kode_induk,'.',mst_akun.kode_akun) LIKE '4.1.99%'))
		");
