			...
			<div class="content-laporan">
				<table class="table table-bordered table-striped tabelContent" border="1">
					...
					<tr>
						...
						<th width="40%" style="text-align: center;">Total (Rp.)</th>
					</tr>
					<!-- ROWS 1 -->
					<tr>
						<th>Aset</th>
					...
					<tr>
						...
						<td>
							...
							?>
									...
									<li style="list-style-type: none;"><?php echo $row->nama_akun?></li>
						...
						<td style="text-align: right; padding-right: 5px;">
							...
							?>
									...
									<li style="list-style-type: none !important;"><?php  echo formatCurrency($total);?></li>
					...
					<tr>
						...
						<th style="text-align:right;"><?php echo formatCurrency($totalasetlancar)?></th>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="textRight" style="text-align: right; padding-right: 5px;">
							<ul style="list-style: none;">
							<?php
								$asetTidakLancar = $this->db->query("SELECT CONCAT(mst_akun.kode_induk,'.',mst_akun.kode_akun) as kodeakun,
								mst_akun.nama_akun
								...
								$total = GetTotalSaldo($KodeAkun, $endDate);
								$totalAset += $total;
								$saldo = $total;// ($saldo->first_row()->total < 0 ) ? "(".formatCurrency($saldo->first_row()->total *-1) .")" : formatCurrency($saldo->first_row()->total);
									
							?>
								<li style="list-style: none;"><?php echo formatCurrency($total);?></li>
							<?php
								$totalasettidaklancar += $total;
								}
							?>
							</ul> 
						</td>
					...
					<tr>
						...
						<th style="text-align:right;"><?php echo formatCurrency($totalasettidaklancar)?></th>
					</tr>
					<!-- rows 10 -->
					...
					<tr>
						...
						<td class="textRight" style="text-align: right; padding-right: 5px;"> 
							...
							?>
									...
									<li><?php echo formatCurrency($total)?></li>
					...
					<tr>
						...
						<th style="text-align:right; padding-right: 10px;"> 
							<?php
								...
								*/
								$totalSumKewajiban = $totalKewajiban + $totalKewajibanpjg;
								echo formatCurrency($totalSumKewajiban);
							?>
						</th>
					</tr>
					<tr>
							...
							<?php
								...
								/*$saldo = $this->db->query("SELECT COALESCE(SUM(mst_saldo_awal.nominal)) as total FROM mst_saldo_awal
									...
									if (in_array($KodeAkun, GetCOAAkunLinkExclude(array('Surplus/Defisit Periode Lalu')) )  )
									{
										...
										echo '<li>'.formatCurrency($totalSDPeriodeLalu).'</li>';
									...
									else if (in_array($KodeAkun, GetCOAAkunLinkExclude(array('Surplus/Defisit Periode Berjalan')) )  )
									{
										...
										echo '<li>'.formatCurrency($totalSDPeriodeBerjalan).'</li>';
									...
									else if (in_array($KodeAkun, GetCOAAkunLinkExclude(array('Saldo Awal')) )  )
									{
										...
										echo '<li>'.formatCurrency($totalSaldoAwal).'</li>';
									}
									else
									{
										...
										echo '<li class="textRight">'.formatCurrency($total).'</li>';
					...
					<tr>
						<?php
							$totalSumKewajibanSaldoDana = $totalSumKewajiban + $totalSaldoDana;
						?>
						<th>Jumlah Kewajiban dan Ekuitas</th>
						<th style="text-align:right; padding-right: 10px;"><?php echo formatCurrency($totalSumKewajibanSaldoDana);?></th>
					</tr>
				</table>
			</div>
		</div>
...
<?php
  if($tes == 'pdf'){
    ...
    $html2pdf->Output('Laporan_neraca_potrait.pdf');
    }
