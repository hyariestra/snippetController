...
<div class="body-laporan">
	...
	<div class="content-laporan">
		<table class="table table-bordered table-striped tabelContent" border="1">
			...
			<tr>
				<td style="font-weight:bold;"><?php echo $setting->first_row()->label_perubahanaset;?> - aset kelolaan lancar</td>
				<td></td>
				<td></td>
				<td></td>
				<td></td>
			...
			foreach($piutang->result_array() as $row){ 
				...
				$pemasukan = $this->db->query("SELECT 
				COALESCE(SUM(trx_jurnal_det.debet_akhir),0) as nominal
				FROM trx_jurnal
				...
				COALESCE(SUM(trx_jurnal_det.kredit_akhir),0) as nominal
				FROM trx_jurnal
				LEFT JOIN trx_jurnal_det ON trx_jurnal.id_jurnal = trx_jurnal_det.id_jurnal
				WHERE trx_jurnal_det.id_akun = '".$row['idakun']."'
				AND trx_jurnal.id_sumber_trans <> 6
				...
				$saldoakhir = ($saldoawal->first_row()->nominal + $pemasukan->first_row()->nominal) - $pengeluaran->first_row()->nominal;
			?>
				<tr>
					<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $seq.". ".$row['namaakun']?></td>
					<td style="text-align:right;"><?php echo formatCurrency(@$saldoawal->first_row()->nominal)?></td>
					<td style="text-align:right;"><?php echo formatCurrency($pemasukan->first_row()->nominal)?></td>
					<td style="text-align:right;"><?php echo formatCurrency($pengeluaran->first_row()->nominal)?></td>
					<td style="text-align:right;"><?php echo formatCurrency(0)?></td>
					<td style="text-align:right;"><?php echo formatCurrency(0)?></td>
					<td style="text-align:right;"><?php echo formatCurrency($saldoakhir)?></td>
				</tr>
			<?php 
			
			$total = (@$saldoawal->first_row()->nominal + $pemasukan->first_row()->nominal) - $pengeluaran->first_row()->nominal;
			
			$totalsaldoawalpiutang += @$saldoawal->first_row()->nominal;
			$totalpenambahanpiutang += $pemasukan->first_row()->nominal;
			$totalpenguranganpiutang += $pengeluaran->first_row()->nominal;
			$totalsaldoakhirpiutang += @$total;
				
			$seq++;
			}?>
			<tr>
				<td style="font-weight:bold;"><?php echo $setting->first_row()->label_perubahanaset;?> - aset kelolaan tidak lancar</td>
				<td></td>
				<td></td>
				<td></td>
				<td></td>
			...
			foreach($asettetap->result_array() as $row){ 
				...
				$akumulasi = $this->db->query("SELECT 
