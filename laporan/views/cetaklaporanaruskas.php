...
<div class="row">
	<div class="col-md-12">
		<div style="text-align:center;margin-bottom: 15px !important;" >
			...
			<span style=""><?php echo $tglHeader;?></span>
		</div>
			...
			<div class="content-laporan">
				<table class="table table-bordered table-striped tabelContent" border="1">
					...
					<?php
						...
						$pendapatan = $this->db->query("SELECT *, SUM(trx_jurnal_det.kredit_akhir) as total FROM trx_jurnal
						...
						WHERE trx_jurnal_det.id_jurnal IN (SELECT trx_jurnal.id_jurnal 
															FROM trx_jurnal
															LEFT JOIN trx_jurnal_det ON trx_jurnal.id_jurnal = trx_jurnal_det.id_jurnal
															WHERE trx_jurnal_det.id_akun IN (SELECT mst_akun.id_akun FROM mst_akun WHERE CONCAT(mst_akun.kode_induk,'.',mst_akun.kode_akun) LIKE '4%')
															)					
						AND NOT trx_jurnal_det.id_akun IN (SELECT mst_akun.id_akun FROM mst_akun WHERE CONCAT(mst_akun.kode_induk,'.',mst_akun.kode_akun) LIKE '1.1.1%')
					...
					<?php
						...
						$pengeluaran = $this->db->query("SELECT *, SUM(trx_jurnal_det.debet_akhir) as total FROM trx_jurnal
						LEFT JOIN trx_jurnal_det ON trx_jurnal.id_jurnal = trx_jurnal_det.id_jurnal
						LEFT JOIN mst_akun ON mst_akun.id_akun = trx_jurnal_det.id_akun 
						WHERE trx_jurnal_det.id_jurnal IN (SELECT trx_jurnal.id_jurnal 
															FROM trx_jurnal
															LEFT JOIN trx_jurnal_det ON trx_jurnal.id_jurnal = trx_jurnal_det.id_jurnal
															WHERE trx_jurnal_det.id_akun IN (SELECT mst_akun.id_akun FROM mst_akun WHERE CONCAT(mst_akun.kode_induk,'.',mst_akun.kode_akun) LIKE '5%')
															)					
						AND NOT trx_jurnal_det.id_akun IN (SELECT mst_akun.id_akun FROM mst_akun WHERE CONCAT(mst_akun.kode_induk,'.',mst_akun.kode_akun) LIKE '1.1.1%')
						AND (trx_jurnal.tgl_jurnal >= '".$startDate."' AND trx_jurnal.tgl_jurnal <= '".$endDate."')
						GROUP BY trx_jurnal_det.id_akun");
					...
					<?php
						$totalArusInvestasi = 0;
						
						
						
						$investasi = $this->db->query("SELECT 
						(COALESCE(SUM(trx_jurnal_det.debet_akhir),0) - COALESCE(SUM(trx_jurnal_det.kredit_akhir),0)) as total,
						mst_akun.nama_akun
						FROM trx_jurnal
						LEFT JOIN trx_jurnal_det ON trx_jurnal.id_jurnal = trx_jurnal_det.id_jurnal
						LEFT JOIN mst_akun ON mst_akun.id_akun = trx_jurnal_det.id_akun
